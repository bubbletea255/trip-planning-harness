---
name: harness-sync
description: Synchronizes core/ agent definitions to platform-specific adapters. Activate when: "어댑터 동기화해줘", "Codex 파일 업데이트해줘", "core 바꿨는데 동기화해줘", "/harness-sync", "플랫폼 파일 재생성해줘". Run this after modifying any file in core/agents/ or core/prompts/ to keep .Codex/agents/ and AGENTS.md in sync.
---

# Harness Sync Skill

## 역할
`core/` 폴더의 Agent 정의를 Codex와 Codex 플랫폼 형식으로 동기화한다.
...
| 소스 | 대상 | 변환 내용 |
|---|---|---|
| `core/agents/*.md` | `.Codex/agents/*.md` | Codex frontmatter 추가 |
...
### Step 2: Codex 어댑터 동기화 (.Codex/agents/)

각 core/agents/{name}.md에 대해:

1. 파일 내용을 읽는다.
2. 해당 Agent의 Codex frontmatter 템플릿을 적용한다.
3. `.Codex/agents/{name}.md`로 덮어쓴다.

**Codex frontmatter 템플릿**:
```markdown
---
name: {agent-name}
...
```
동기화 완료 보고 형식:
  ✓ .Codex/agents/intake.md
  ✓ .Codex/agents/researcher.md
  ✓ .Codex/agents/planner.md
  ✓ .Codex/agents/budget-analyst.md
  ✓ .Codex/agents/report-composer.md
  ✓ .Codex/agents/judge.md
  ✓ AGENTS.md (Codex 어댑터)
...
- `.Codex/agents/` 파일을 직접 편집했다면 `core/`에 먼저 반영한 뒤 동기화한다.
...
동기화 후 아래를 확인한다.
- [ ] core/agents/ 파일 수 = .Codex/agents/ 파일 수
- [ ] AGENTS.md에 6개 Agent 섹션 모두 포함됨
- [ ] 각 .Codex/agents/ 파일에 frontmatter가 있음
