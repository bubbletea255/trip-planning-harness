...
| 비교 모드 결과 (`*-Codex/`, `*-codex/`) 존재 | Judge Agent 단독 실행 가능 |
...
- 비교 모드: `run-YYYYMMDD-{목적지약어}-Codex` / `run-YYYYMMDD-{목적지약어}-codex`
...
`{run-id}-Codex/05-report.html`과 `{run-id}-codex/05-report.html`이 모두 존재하는지 확인한다.
